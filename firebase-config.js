...
const FALLBACK_IMAGES = {
  ...
  'assets/sala-2.jpeg':'sala-2.jpeg',
  'assets/sala-3.jpeg':'sala-3.jpeg',
  'assets/sala-4.jpeg':'sala-4.jpeg',
  'assets/sala-5.jpeg':'sala-5.jpeg',
  'assets/sala-6.jpeg':'sala-6.jpeg',
...
const roomsSeed = [
  ...
  { id:'sala-2', name:'Sala 2', capacity:4, image:'assets/sala-2.jpeg', images:['assets/sala-2.jpeg'], location:'Administrativo', active:true, resources:'Mesa, cadeiras e espaço reservado' },
  { id:'sala-3', name:'Sala 3', capacity:4, image:'assets/sala-3.jpeg', images:['assets/sala-3.jpeg'], location:'Administrativo', active:true, resources:'Mesa, cadeiras e espaço reservado' },
  ...
  { id:'sala-6', name:'Sala 6', capacity:8, image:'assets/sala-6.jpeg', images:['assets/sala-6.jpeg'], location:'Administrativo', active:true, resources:'Mesa executiva, cadeiras e espaço reservado' },
...
const usersSeed = [
  ...
  { id:'murillo-admin', name:'Murillo Netto', email:'murillo.netto@ultrafarma.com.br', password:'admin123', role:'admin', status:'approved', department:'Marketing' },
...
const addDaysISO = (iso, days) => { const d=new Date(iso+'T12:00:00'); d.setDate(d.getDate()+days); return d.toISOString().slice(0,10); };
const endDateOf = r => r.endDate || r.date;
const reservationCoversDate = (r, iso) => r.date <= iso && endDateOf(r) >= iso;
const reservationPeriodLabel = r => endDateOf(r)===r.date ? new Date(r.date+'T12:00:00').toLocaleDateString('pt-BR') : `${new Date(r.date+'T12:00:00').toLocaleDateString('pt-BR')} até ${new Date(endDateOf(r)+'T12:00:00').toLocaleDateString('pt-BR')}`;
const datesBetween = (start, end) => { const arr=[]; for(let d=start; d<=end; d=addDaysISO(d,1)) arr.push(d); return arr; };
...
function ensureSeeds(base){
  ...
  const requiredAdmins = usersSeed.filter(u => u.role === 'admin');
  const missingAdmins = requiredAdmins.filter(seed => !users.some(u => String(u.email||'').toLowerCase() === seed.email.toLowerCase()));
  const mergedUsers = [...missingAdmins, ...users];
...
function currentReservations(){ return data.reservations.filter(r => reservationCoversDate(r,state.date) && (state.room==='all' || r.roomId===state.room) && r.status!=='cancelled'); }
function approvedRooms(){ return data.rooms.filter(r=>r.active); }
function hasConflict(roomId,startDate,endDate,start,end,ignoreId=null){
  const s=minutes(start), e=minutes(end);
  return data.reservations.some(r =>
    r.id!==ignoreId &&
    r.roomId===roomId &&
    r.status!=='cancelled' &&
    startDate <= endDateOf(r) &&
    endDate >= r.date &&
    Math.max(s,minutes(r.start)) < Math.min(e,minutes(r.end))
  );
}
...
function renderRoomDay(room){
  const res = data.reservations.filter(r=>r.roomId===room.id && reservationCoversDate(r,state.date) && r.status!=='cancelled').sort((a,b)=>minutes(a.start)-minutes(b.start));
...
function nextFree(roomId,date){
  ...
  for(let s=start; s+30<=endDay; s+=30){ if(!hasConflict(roomId,date,date,hm(s),hm(s+30))) return hm(s); }
  return 'Sem horário';
}
function renderReservationRow(r){
  const user=getUser(r.userId);
  return `<div class="reservation-row"><div class="time">${r.start}</div><div><strong>${escapeHtml(r.title)}</strong><br><span class="muted">${r.start}–${r.end} • ${reservationPeriodLabel(r)} • ${r.people} pessoa(s) • ${escapeHtml(user.name)}</span></div><div>${(isAdmin() || r.userId===state.currentUser.id)?`<button class="btn-danger btn-small" onclick="cancelReservation('${r.id}')">Cancelar</button>`:''}</div></div>`;
}
...
function renderWeekSlot(date,time){
  const s=minutes(time), e=s+60; const list=data.reservations.filter(r=>reservationCoversDate(r,date) && (state.room==='all'||r.roomId===state.room) && r.status!=='cancelled' && Math.max(s,minutes(r.start))<Math.min(e,minutes(r.end)));
...
function roomStatus(roomId,m){
  const res=data.reservations.filter(r=>r.roomId===roomId && reservationCoversDate(r,state.date) && r.status!=='cancelled');
...
function renderReservationsAdmin(){
  return `<div class="section-title"><h2>Gestão de reservas</h2><button class="btn-primary" onclick="openReservation()">+ Reserva</button></div><div class="table-wrap"><table class="table"><thead><tr><th>Período</th><th>Horário</th><th>Sala</th><th>Título</th><th>Pessoa</th><th>Ações</th></tr></thead><tbody>${data.reservations.filter(r=>r.status!=='cancelled').sort((a,b)=>(a.date+a.start).localeCompare(b.date+b.start)).map(r=>`<tr><td>${reservationPeriodLabel(r)}</td><td>${r.start}–${r.end}</td><td>${getRoom(r.roomId).name}</td><td>${escapeHtml(r.title)}</td><td>${escapeHtml(getUser(r.userId).name)}</td><td><button class="btn-danger btn-small" onclick="cancelReservation('${r.id}')">Cancelar</button></td></tr>`).join('')}</tbody></table></div>`;
}
...
function renderRoomDetailsModal(){
  ...
  const dayReservations=data.reservations.filter(r=>r.roomId===room.id && reservationCoversDate(r,state.date) && r.status!=='cancelled').sort((a,b)=>minutes(a.start)-minutes(b.start));
  const active=st.res;
...
function renderReservationDetail(r){
  ...
  return `<div class="reservation-detail"><div class="time">${r.start}–${r.end}</div><strong>${escapeHtml(r.title)}</strong><br><span class="muted">Período: ${reservationPeriodLabel(r)}</span><br><span class="muted">Reservado por: ${escapeHtml(user.name)} • ${escapeHtml(user.department||'Sem departamento')}</span><br><span class="muted">Duração diária: ${dur} min • Participantes: ${r.people}</span>${r.notes?`<p>${escapeHtml(r.notes)}</p>`:''}</div>`;
}
...
function renderReservationModal(){
  ...
  return `<div class="modal-backdrop"><div class="modal"><div class="modal-head"><h3>Nova reserva</h3><button class="close" onclick="closeModal()">×</button></div><form class="form" onsubmit="saveReservation(event)"><div class="field"><label>Sala/espaço</label><select required name="roomId"><option value="">Selecione</option>${roomOptions}</select></div><div class="field"><label>Título da reunião</label><input required name="title" placeholder="Ex.: Reunião de Marketing"></div><div class="grid two"><div class="field"><label>Data inicial</label><input required type="date" name="date" value="${state.date}"></div><div class="field"><label>Data final</label><input required type="date" name="endDate" value="${state.date}"></div></div><div class="grid two"><div class="field"><label>Início</label><input required type="time" name="start" value="09:00"></div><div class="field"><label>Fim</label><input required type="time" name="end" value="10:00"></div></div><div class="notice">Para reservas entre datas, o sistema bloqueia o mesmo horário em todos os dias do período selecionado.</div><div class="field"><label>Quantidade de pessoas</label><input required type="number" min="1" name="people" value="2"></div><div class="field"><label>Observações</label><textarea name="notes" placeholder="Equipamentos, pauta ou detalhes adicionais"></textarea></div><button class="btn-primary">Confirmar reserva</button></form></div></div>`;
}
window.saveReservation=e=>{
  e.preventDefault(); const fd=new FormData(e.target); const room=getRoom(fd.get('roomId')); const start=fd.get('start'), end=fd.get('end'), date=fd.get('date'); const endDate=fd.get('endDate') || date; const people=Number(fd.get('people'));
  if(endDate < date) return toast('A data final precisa ser igual ou maior que a data inicial.');
  ...
  if(minutes(end)-minutes(start)<data.settings.minDuration || minutes(end)-minutes(start)>data.settings.maxDuration) return toast(`Duração permitida: ${data.settings.minDuration} a ${data.settings.maxDuration} minutos por dia.`);
  if(people>room.capacity) return toast(`A capacidade de ${room.name} é de ${room.capacity} lugares.`);
  if(datesBetween(date,endDate).length > 31 && !confirm('Você selecionou mais de 31 dias. Deseja confirmar essa reserva de longo período?')) return;
  if(hasConflict(room.id,date,endDate,start,end)) return toast('Conflito de horário em uma ou mais datas desse período. Escolha outro período.');
  data.reservations.push({id:crypto.randomUUID(), roomId:room.id, userId:state.currentUser.id, title:fd.get('title'), date, endDate, start, end, people, notes:fd.get('notes'), status:'approved'}); save(); state.date=date; state.room=room.id; state.modal=null; render(); toast(endDate===date?'Reserva confirmada.':'Reserva entre datas confirmada.');
};
